...
ServiceWrapper.prototype.init = function(){
    ...
    if (this.initialized)
        return Q.fulfill();
    // Init
    return (this.service.init ? Q.mcall(this.service, 'init') : Q.fulfill())
        .then(function(){
            this.emit('init');
...
ServiceWrapper.prototype.start = function(){
    ...
    if (this.running)
        return Q.fulfill();
    // Init
    return (this.service.start? Q.mcall(this.service, 'start') : Q.fulfill() )
        .then(function(){
            this.emit('start');
            this.running = true;
        }.bind(this));
...
ServiceWrapper.prototype.stop = function(){
    ...
    if (!this.running)
        return Q.fulfill();
    // Init
    return (this.service.stop? Q.mcall(this.service, 'stop') : Q.fulfill() )
        .then(function(){
            this.emit('stop');
            this.running = false;
        }.bind(this));
